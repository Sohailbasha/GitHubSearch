...
class APIService {
    ...
    let userInfoBaseURL = "https://api.github.com/users/" // for userInfo
        ...
        func getData(with user: FetchedUser) -> AnyPublisher<User, Error> {
        ...
        func getAllData(query: String) -> AnyPublisher<[User], Error> {
            fetchUsers(query: query).flatMap { fetchedUsers in
                Publishers.Sequence(sequence: fetchedUsers.map { self.getData(with: $0) })
                    .flatMap{$0}
                    .collect()
